...
export default function ShareDocumentModal({ open, documentId, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([listOffices(), getDocumentShares(documentId)])
      .then(([all, shares]) => {
        if (!alive) return;
        setOffices(all.sort((a, b) => a.name.localeCompare(b.name)));
        setSelectedIds(shares.office_ids ?? []);
      })
      .catch((e: any) => { if (alive) setError(e?.message ?? "Failed to load share settings."); })
