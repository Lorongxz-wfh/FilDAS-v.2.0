  ...
  <section className="flex flex-1 flex-col gap-3 h-full">
    ...
    {items.length === 0 ? (
      <div className="flex flex-1 h-full">
        <div className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-6 py-8 text-center flex items-center justify-center dark:border-surface-400 dark:bg-surface-600">
          <div>
            <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
              ✓
            </div>
            <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              All caught up
      ...
      items.map((it) => {
        ...
        return (
          <div
            key={`${doc.id}-${ver.id}`}
            className="group hover:shadow-md transition-shadow p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 dark:border-surface-400 dark:hover:bg-surface-400"
